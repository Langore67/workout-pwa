...
export function formatCoachExportText(metrics: CoachExportMetrics) {
  const lines = [
    ...
    `- Bodyweight used by strength engine: ${formatValue(metrics.strengthSignal.currentBodyweight, 1, " lb")} (${metrics.strengthSignal.bodyweightDaysUsed ?? "Unknown"} day avg sample)`,
    "",
    "Anchor Lifts",
