...
export default function TrendChartCard({
  ...
  const renderHeader = () => (
    <div className="grid" style={{ rowGap: 0, marginBottom: 4 }}>
      <div
        data-testid={`${title}:header-grid`}
        className="w-full"
        style={{
          display: "flex",
          width: "100%",
      ...
      >
        ...
        {showHeaderStatus ? (
          ...
          >
            <div
              data-testid={`${title}:status-content`}
              style={{
                display: "flex",
                flexShrink: 0,
                alignItems: "center",
  ...
  return (
    <div className="min-w-0 rounded-2xl border border-[var(--line)] bg-[var(--card)] p-4 shadow-sm">
      ...
      {resolvedReadoutMode === "statRow" ? (
        <div className="mb-4 grid" style={{ rowGap: 2 }}>
          {latestDisplayLabel ? (
          ...
          >
            ...
            {statSecondary.trim() ? (
              <span className="inline-flex shrink-0 items-center rounded-full border border-[var(--line)] px-2.5 py-1 text-[11px] font-medium text-[var(--muted)] leading-none whitespace-nowrap">
                {statSecondary.trim()}
              </span>
            ) : null}
          </div>
            ...
            >
              {hasRangeSummary ? (
                <span className="rounded-full border border-[var(--line)] px-2 py-0.5">
                  {`${firstVisibleLabel} â†’ ${lastVisibleLabel}`}
                </span>
              ) : null}
              {hasDeltaSummary ? (
                <span className="rounded-full border border-[var(--line)] px-2 py-0.5">
