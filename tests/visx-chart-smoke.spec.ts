...
async function seedSharedStrengthTrendData(page: Page) {
  return await page.evaluate(async () => {
    ...
    const exercises = [
      { name: "Back Squat", equipment: "Barbell", category: "Barbell", pattern: "squat", baseWeight: 225 },
      { name: "Romanian Deadlift", equipment: "Barbell", category: "Barbell", pattern: "hinge", baseWeight: 245 },
      { name: "Bench Press", equipment: "Barbell", category: "Barbell", pattern: "push", baseWeight: 185 },
      { name: "Pull Up", equipment: "Bodyweight", category: "Bodyweight", pattern: "pull", baseWeight: 0 },
    ];
...
async function expectRenderedVisxTrendChart(page: Page, title: string) {
  const chart = page.locator(`svg[aria-label="${title}"]`).first();
